...
it('request status should be PUT', function(done) {
var data = { foo: 'bar' };
var dataJson = JSON.stringify(data);

sendRequest('PUT', '/posts/1', data, function(err, result) {
  done();
});
this.requests[0].method.should.equal('PUT');
this.requests[0].respond(200, { 'Content-Type': 'text/json' }, dataJson);
  ...
  sendRequest('PUT', '/posts/1', data, function() {
    done();
  });
  ...
  sendRequest('PUT', '/posts/1', '', function(err, result) {
    done();
  });
